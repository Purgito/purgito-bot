// Dashboard por servidor del rediseño (/es/dashboard/:id): header con datos
// del guild, tabs INICIO/CHAT/GIFS/MEMES/EMBEDS/PREMIUM y el contenido de las
// tabs nuevas (INICIO y CHAT tienen guardado automático con toast). GIFS,
// EMBEDS y PREMIUM reutilizan los loaders ya existentes del panel.
...
import { loadEmbeds, panelModal, getEmojis, uploadImageBlob } from './embeds/shared-ui.js';
...
async function loadHead() {
  ...
  try {
    ...
    head.innerHTML = '';
    head.append(
      el('a', { class: 'dash-back dim', href: `/${currentLocale()}/perfil` }, '← Volver atrás'),
  ...
  } catch (e) { /* header decorativo, no bloquea el dashboard */ }
}
...
async function loadInicio() {
  ...
  try {
    ...
    const tiles = el('div', { class: 'stat-grid' },
      statTile('corpus', stats.corpus_total, 'Mensajes aprendidos'),
      statTile('smile', stats.reactions, 'Reacciones en el pool'),
      statTile('film', stats.gifs, 'GIFs guardados'),
      statTile('sparkle', stats.frases, 'Frases especiales'),
      statTile('info', stats.member_count, 'Miembros'));
    ...
    box.append(formGroup('Resumen del servidor', tiles,
      stats.corpus_by_channel.length
        ? el('div', {}, el('p', { class: 'dim' }, 'Mensajes aprendidos por canal:'), byChannel)
        : null));
