...
def test_list_audit_log_filters_and_search(memory_db):
    async def run():
        ...
        await db.log_audit(guild_id, 101, "Frambuesa", "frases.add", "frase especial de bienvenida")
        await db.log_audit(guild_id, 101, "Frambuesa", "frases.edit", "frase editada de despedida")
        await db.log_audit(guild_id, 102, "Ana", "gifs.add", "https://media.giphy.com/media/test/giphy.gif")
        await db.log_audit(guild_id, 102, "Ana", "gifs.block", "https://tenor.com/view/bad-gif")
        await db.log_audit(guild_id, 103, "Carlos", "chat.settings_update", "chance=25")
        await db.log_audit(guild_id, 103, "Carlos", "youtube.add", "Canal de Tutoriales")
        await db.log_audit(other_guild, 999, "Intruso", "frases.add", "frase otro server")
        ...
        entries, has_more = await db.list_audit_log_page(guild_id, q="texto_inexistente_123")
...
def test_audit_log_pagination_with_cursor_and_filters(memory_db):
    async def run():
        ...
        for i in range(1, 16):
            await db.log_audit(guild_id, 101, "Frambuesa", "frases.add", f"frase #{i:02d}")

        # Página 1 (5 items)
        page1, has_more1 = await db.list_audit_log_page(guild_id, limit=5, action="frases.")
        assert len(page1) == 5
        ...
        cursor1 = page1[-1]["id"]
        page2, has_more2 = await db.list_audit_log_page(guild_id, before_id=cursor1, limit=5, action="frases.")
        assert len(page2) == 5
        ...
        # Página 3
        cursor2 = page2[-1]["id"]
        page3, has_more3 = await db.list_audit_log_page(guild_id, before_id=cursor2, limit=5, action="frases.")
        assert len(page3) == 5
...
def test_api_audit_log_endpoint_filters_and_users(memory_db, monkeypatch):
    ...
    async def run():
        guild_id = 12345
        await db.log_audit(guild_id, 101, "Frambuesa", "frases.add", "frase 1")
        await db.log_audit(guild_id, 102, "Ana", "gifs.add", "gif 1")

        # 1. GET sin query (página inicial) -> devuelve entries, has_more y lista de users
        ...
        # 3. GET con filtro de usuario
        req_user = FakeRequest(guild_id=guild_id, query={"user_id": "101"})
        ...
        req_date = FakeRequest(guild_id=guild_id, query={"date_from": "2020-01-01", "date_to": "2030-01-01"})
